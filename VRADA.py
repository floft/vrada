...
if __name__ == '__main__':
    ...
    parser.add_argument('--shl', dest='shl', action='store_true',
        help="Run on the Sussex-Huawei Locomotion dataset")
    parser.add_argument('--no-shl', dest='shl', action='store_false',
        help="Do not run on the Sussex-Huawei Locomotion dataset (default)")
    ...
    parser.set_defaults(
        ...
        svhn=False, mnist=False, shl=False, balance=True, bidirectional=False,
        debug=False)
    ...
    assert args.mimic_ahrf + args.mimic_icd9 + \
        ...
        + args.svhn + args.mnist + args.shl == 1, \
    ...
    if args.trivial_line:
        ...
        test_data_a, test_labels_a, \
        train_data_b, train_labels_b, \
        test_data_b, test_labels_b = load_data_shl(
            "datasets/SHL/train",
            "datasets/SHL/test")

        # Information about dataset
        index_one = True # Labels start from 1
        num_features = train_data_a.shape[2]
        time_steps = train_data_a.shape[1]
        num_classes = len(np.unique(train_labels_a))
        multi_class = False # Predict only one class
